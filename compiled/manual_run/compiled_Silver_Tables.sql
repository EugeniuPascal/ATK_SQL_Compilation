...
----------------------------------------------------------------------------------------------------
-- Start of: mis.Silver_Events.sql
----------------------------------------------------------------------------------------------------
BEGIN
    SET @StartTime = GETDATE();
    SET @Status = 'Running';
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Events', @StartTime, @EndTime, @Status, @FailureNote);
END

----------------------------------------------------------------------------------------------------
-- End of: mis.Silver_Events.sql
----------------------------------------------------------------------------------------------------

----------------------------------------------------------------------------------------------------
-- Start of: mis.Silver_Clients_base.sql
----------------------------------------------------------------------------------------------------
BEGIN
    SET @StartTime = GETDATE();
    SET @Status = 'Running';
...
/*==============================================================
  DROP + CREATE TABLE
==============================================================*/
IF OBJECT_ID(N''mis.Silver_Clients_base'', ''U'') IS NOT NULL
    DROP TABLE mis.Silver_Clients_base;

CREATE TABLE mis.Silver_Clients_base
(
    ...
    CONSTRAINT PK_Silver_Clients_base
...
-- Insert final data
INSERT INTO mis.[Silver_Clients_base]
SELECT
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Clients_base', @StartTime, @EndTime, @Status, @FailureNote);
END

----------------------------------------------------------------------------------------------------
-- End of: mis.Silver_Clients_base.sql
----------------------------------------------------------------------------------------------------

----------------------------------------------------------------------------------------------------
-- Start of: mis.Silver_EmployeesPosition_SCD.sql
----------------------------------------------------------------------------------------------------
BEGIN
    SET @StartTime = GETDATE();
    SET @Status = 'Running';
...
INCLUDE(PositionID, PositionName, BranchName);';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_EmployeesPosition_SCD', @StartTime, @EndTime, @Status, @FailureNote);
END

----------------------------------------------------------------------------------------------------
-- End of: mis.Silver_EmployeesPosition_SCD.sql
----------------------------------------------------------------------------------------------------
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_CerereOnline_base', @StartTime, @EndTime, @Status, @FailureNote);
END

----------------------------------------------------------------------------------------------------
-- End of: mis.Silver_CerereOnline_base.sql
----------------------------------------------------------------------------------------------------
...
GROUP BY m.ClientID, d.SoldDate;';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Client_UnhealedFlag', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Resp_SCD', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Stages_SCD', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_SCD_GroupMembershipPeriods', @StartTime, @EndTime, @Status, @FailureNote);
END
...
INCLUDE (ClientID, SoldAmount, GroupOwner, BranchID);';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Sold_Owner', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Limits', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_Conditions_After_Disb', @StartTime, @EndTime, @Status, @FailureNote);
END
...
';
    ...
    SET @EndTime = GETDATE();
    INSERT INTO mis.Silver_Proc_Exec_Log (TableName, StartTime, EndTime, Status, Failure_Note)
    VALUES ('mis.Silver_CPD_TaskDays', @StartTime, @EndTime, @Status, @FailureNote);
END
