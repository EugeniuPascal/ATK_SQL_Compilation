...
(
    ...
    FROM [ATK].[mis].[2tbl_Gold_Sold_Owner] f
);

IF @AsOfDate IS NULL
    THROW 50002, 'Sold is empty: [ATK].[mis].[2tbl_Gold_Sold_Owner].', 1;

------------------------------------------------------------
-- 1) Подготовка: добавляем колонки (если нет) + чистим таблицу
------------------------------------------------------------
IF COL_LENGTH('mis.2tbl_Gold_Fact_CPD_Sold_v2', 'CreditRisk') IS NULL
    EXEC('ALTER TABLE [mis].[2tbl_Gold_Fact_CPD_Sold_v2] ADD [CreditRisk] varchar(64) NULL;');

IF COL_LENGTH('mis.2tbl_Gold_Fact_CPD_Sold_v2', 'LegalRisk') IS NULL
    EXEC('ALTER TABLE [mis].[2tbl_Gold_Fact_CPD_Sold_v2] ADD [LegalRisk] varchar(64) NULL;');

IF COL_LENGTH('mis.2tbl_Gold_Fact_CPD_Sold_v2', 'BranchID1') IS NULL
    EXEC('ALTER TABLE [mis].[2tbl_Gold_Fact_CPD_Sold_v2] ADD [BranchID1] varchar(36) NULL;');

IF COL_LENGTH('mis.2tbl_Gold_Fact_CPD_Sold_v2', 'BranchID2') IS NULL
    EXEC('ALTER TABLE [mis].[2tbl_Gold_Fact_CPD_Sold_v2] ADD [BranchID2] varchar(36) NULL;');

TRUNCATE TABLE [ATK].[mis].[2tbl_Gold_Fact_CPD_Sold_v2];
...
INTO #need_dates
FROM [ATK].[mis].[2tbl_Silver_CPD_TaskDays] td;
...
INSERT INTO [ATK].[mis].[2tbl_Gold_Fact_CPD_Sold_v2]
(
...
WHERE
 ...
 OR (
        ...
        (
            SELECT 1
            FROM [ATK].[mis].[2tbl_Gold_Fact_CPD_Sold_v2] x
