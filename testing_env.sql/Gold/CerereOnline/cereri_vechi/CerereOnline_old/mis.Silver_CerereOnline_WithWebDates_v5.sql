...
SET
    ...
    , t.[Tip Рассмотрения Заявки RO] =
        CASE
            ...
            WHEN COALESCE(d.Tip Рассмотрения Заявки, t.[Tip Рассмотрения Заявки]) = N'БезЗвонка'   THEN N'Fara sunet'
            WHEN COALESCE(d.Tip Рассмотрения Заявки, t.[Tip Рассмотрения Заявки]) = N'Стандартный' THEN N'Standart'
            ELSE NULL
        END

    -- 08-20 (старые)
    , t.[Viteza de decizie] =
        mis.fn_WorkMinutesSigned(COALESCE(d.Data depunerii cererii, CAST(t.[Data depunerii cererii] AS datetime2(0))),
    ...
    , t.[Viteza de procesare] =
        mis.fn_WorkMinutesSigned(COALESCE(d.Data procesarii, CAST(t.[Data procesarii] AS datetime2(0))),
                                 CAST(COALESCE(v.VoteDate, t.[Data Votarii]) AS datetime2(0)),
                                 8*60, 20*60)
    ...
    , t.[Analyse] =
        mis.fn_WorkMinutesSigned(COALESCE(d.Data depunerii cererii,  CAST(t.[Data depunerii cererii] AS datetime2(0))),
                                 COALESCE(d.Data inaintare la CC, CAST(t.[Data inaintare la CC] AS datetime2(0))),
                                 8*60, 18*60)
    ...
    , t.[Viteza de votare CC] =
        CASE
            ...
            THEN mis.fn_WorkMinutesSigned(COALESCE(d.Data inaintare la CC, CAST(t.[Data inaintare la CC] AS datetime2(0))),
                                          CAST(COALESCE(v.VoteDate, t.[Data Votarii]) AS datetime2(0)),
                                          9*60, 18*60)
            ELSE mis.fn_WorkMinutesSigned(COALESCE(d.Data inaintare la CC, CAST(t.[Data inaintare la CC] AS datetime2(0))),
                                          CAST(COALESCE(v.VoteDate, t.[Data Votarii]) AS datetime2(0)),
                                          8*60, 17*60)
        END

    , t.[CC] =
        mis.fn_WorkMinutesSigned(COALESCE(d.Data inaintare la CC, CAST(t.[Data inaintare la CC] AS datetime2(0))),
                                 CAST(t.[CommitteeDecisionDate] AS datetime2(0)),
                                 8*60, 18*60)
    ...
    , t.[Timpul de asteptare] =
        mis.fn_WorkMinutesSigned(COALESCE(d.Data depunerii cererii,      CAST(t.[Data depunerii cererii] AS datetime2(0))),
                                 COALESCE(d.Data procesarii, CAST(t.[Data procesarii] AS datetime2(0))),
                                 9*60, 18*60)
    ...
    , t.[Depasire norma viteza] =
        CASE
            WHEN mis.fn_WorkMinutesSigned(COALESCE(d.Data depunerii cererii, CAST(t.[Data depunerii cererii] AS datetime2(0))),
                                          CAST(COALESCE(v.VoteDate, t.[Data Votarii]) AS datetime2(0)),
            ...
            WHEN (
                 ...
                 AND mis.fn_WorkMinutesSigned(COALESCE(d.Data depunerii cererii, CAST(t.[Data depunerii cererii] AS datetime2(0))),
                                              CAST(COALESCE(v.VoteDate, t.[Data Votarii]) AS datetime2(0)),
