...
CREATE TABLE mis.[2tbl_Gold_Dim_Credits1] (
    ...
    [First_PartnerName_2023] NVARCHAR(255) NULL,
    [First_PartnerName_2024] NVARCHAR(255) NULL,
    [First_PartnerName_2025] NVARCHAR(255) NULL
);
GO
...
-- First partner propagation
FirstPartner AS (
    SELECT
        c.[Кредиты Владелец] AS Owner,
...
-- First partner propagation with per-year logic
FirstPartnerPropagated AS (
    SELECT
        f.CreditID,
        f.Owner,
        f.PartnerID,
        f.PartnerName,
        f.[Кредиты Дата Выдачи],

        -- Overall first partner (all-time)
        CASE 
            WHEN ISNULL(MIN(CASE WHEN f.[Кредиты Дата Выдачи] > '1753-01-01' THEN f.PartnerID END)
                 OVER(PARTITION BY f.Owner), '00000000000000000000000000000000') = '00000000000000000000000000000000'
                 THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] > '1753-01-01' THEN f.PartnerID END)
        ...
        CASE 
            WHEN ISNULL(MIN(CASE WHEN f.[Кредиты Дата Выдачи] > '1753-01-01' THEN f.PartnerID END)
                 OVER(PARTITION BY f.Owner), '00000000000000000000000000000000') = '00000000000000000000000000000000'
                 THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] > '1753-01-01' THEN f.PartnerName END)
                 OVER(PARTITION BY f.Owner)
        END AS First_PartnerName,

        -- First partner per year 2023
        CASE
            WHEN ISNULL(MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2023-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner), '00000000000000000000000000000000')
                 = '00000000000000000000000000000000'
            THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2023-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner)
        ...
        CASE
            ...
            THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2023-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerName END) OVER(PARTITION BY f.Owner)
        END AS First_PartnerName_2023,

        -- First partner per year 2024
        CASE
            WHEN ISNULL(MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2024-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner), '00000000000000000000000000000000')
                 = '00000000000000000000000000000000'
            THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2024-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner)
        ...
        CASE
            ...
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2024-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerName END) OVER(PARTITION BY f.Owner)
        END AS First_PartnerName_2024,

        -- First partner per year 2025
        CASE
            WHEN ISNULL(MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2025-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner), '00000000000000000000000000000000')
                 = '00000000000000000000000000000000'
            THEN NULL
            ELSE MIN(CASE WHEN f.[Кредиты Дата Выдачи] >= '2025-01-01' AND f.[Кредиты Дата Выдачи] > '1753-01-01'
                         THEN f.PartnerID END) OVER(PARTITION BY f.Owner)
...
INSERT INTO mis.[2tbl_Gold_Dim_Credits1] (
    ...
    [First_PartnerName_2023], [First_PartnerName_2024], [First_PartnerName_2025]
)
SELECT
    ...
    cr.PartnerID,
CASE WHEN c.[Кредиты Дата Выдачи] = '1753-01-01' THEN NULL ELSE fpP.First_PartnerID END AS First_PartnerID,
CASE WHEN c.[Кредиты Дата Выдачи] = '1753-01-01' THEN NULL ELSE fpP.First_PartnerName END AS First_PartnerName,
...
COALESCE(resp.LastEmployeeID, cr.EmployeeID) AS LastEmployeeID,
   ...
   CASE cr.Source
    ...
    seg.SegmentRevenue,
    gc.GreenCredit, gc.CommitteeProt_CrPurpose, 
    ...
    e.[СекторыЭкономики Сектор Экономики EFSE] AS EconomicSectorEFSE, c.[Кредиты Сектор Экономики],     
	CASE
        WHEN fp.FinancialProductsMainGroup = 'Business' AND c.[Кредиты Сектор Экономики] = '1. Agricultura'
        THEN 'Agro'
        ELSE 'NonAgro'
    END AS Agro,
-- First partner per year with IssueDate check
CASE WHEN c.[Кредиты Дата Выдачи] >= '2023-01-01' THEN fpP.First_PartnerID_2023 ELSE NULL END AS First_PartnerID_2023,
CASE WHEN c.[Кредиты Дата Выдачи] >= '2024-01-01' THEN fpP.First_PartnerID_2024 ELSE NULL END AS First_PartnerID_2024,
CASE WHEN c.[Кредиты Дата Выдачи] >= '2025-01-01' THEN fpP.First_PartnerID_2025 ELSE NULL END AS First_PartnerID_2025,

CASE WHEN c.[Кредиты Дата Выдачи] >= '2023-01-01' THEN fpP.First_PartnerName_2023 ELSE NULL END AS First_PartnerName_2023,
CASE WHEN c.[Кредиты Дата Выдачи] >= '2024-01-01' THEN fpP.First_PartnerName_2024 ELSE NULL END AS First_PartnerName_2024,
CASE WHEN c.[Кредиты Дата Выдачи] >= '2025-01-01' THEN fpP.First_PartnerName_2025 ELSE NULL END AS First_PartnerName_2025

FROM Credits c
LEFT JOIN CreditRequest cr ON c.[Кредиты ID] = cr.CreditID
LEFT JOIN Resp resp ON c.[Кредиты ID] = resp.CreditID
...
LEFT JOIN FirstPartnerPropagated fpP ON c.[Кредиты ID] = fpP.CreditID
