...
IF OBJECT_ID(N'mis.[Gold_Dim_Credits_v3]', 'U') IS NOT NULL
    DROP TABLE mis.[Gold_Dim_Credits_v3];
GO

CREATE TABLE mis.[Gold_Dim_Credits_v3] (
    ...
    [CreditPartnerName] NVARCHAR(255) NULL,   -- <-- NEW COLUMN
...
CreditRequest AS (
    ...
    FROM (
        SELECT
            ...
            znk.[ЗаявкаНаКредит Клиент ID] AS ClientID,
...
-- ✅ New CTE: first non-null partner per client
ClientPartner AS (
    SELECT DISTINCT
        znk.[ЗаявкаНаКредит Клиент ID] AS ClientID,
        FIRST_VALUE(znk.[ЗаявкаНаКредит Партнер]) OVER (
            PARTITION BY znk.[ЗаявкаНаКредит Клиент ID]
...
INSERT INTO mis.[Gold_Dim_Credits_v3] (
    ...
    [CreditApplicationPartnerID], [CreditPartnerName],
    [FirstFilialID], [FirstEmployeeID],
...
SELECT
    ...
    cp.PartnerName AS CreditPartnerName,
...
LEFT JOIN ClientPartner cp ON cr.ClientID = cp.ClientID
...
LEFT JOIN [ATK].[dbo].[Справочники.СекторыЭкономики] AS e 
       ON c.[Кредиты Сектор Экономики ID] = e.[СекторыЭкономики ID];
