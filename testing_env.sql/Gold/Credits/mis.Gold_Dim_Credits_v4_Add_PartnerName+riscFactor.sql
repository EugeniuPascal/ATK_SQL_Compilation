...
IF OBJECT_ID(N'mis.[Gold_Dim_Credits1]', 'U') IS NOT NULL
    DROP TABLE mis.[Gold_Dim_Credits1];
GO

CREATE TABLE mis.[Gold_Dim_Credits1] (
...
-- Latest OIA row per Application
OIA_LatestPerApp AS (
    SELECT *
    FROM (
        SELECT oia.*,
...
FinalData AS (
    SELECT
        ...
        CASE WHEN crd.[Кредиты Источник Подписания] IS NOT NULL THEN 'True' ELSE 'False' END AS DigitalSign,
        ...
        CASE 
           ...
           ELSE 'Non-Formal'
        END AS IsFormal,
        cr.rn  -- keep rn here for filtering only
    FROM Credits crd
...
INSERT INTO mis.[Gold_Dim_Credits1] (
