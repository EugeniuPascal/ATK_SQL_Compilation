...
============================ */
/* ============================
   Partner propagation strictly by rules
============================ */
WITH FirstPartner AS (
    SELECT
        d.[ДанныеКредитовВыданных Кредит ID] AS CreditID,
        k.[Кредиты Владелец] AS Owner,
        d.[ДанныеКредитовВыданных Дата Выдачи] AS DisbursementDate,
...
Propagated AS (
    SELECT *,
        -- propagate first real partner forward per Owner (Rule 3)
        ...
        -- yearly propagation respecting your rules
        CASE 
            WHEN DisbursementDate >= '2023-01-01' AND PartnerID_Valid IS NOT NULL THEN
                FIRST_VALUE(PartnerID_Valid) OVER (
                    PARTITION BY Owner
                    ORDER BY DisbursementDate, CreditID
                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        ...
        CASE 
            WHEN DisbursementDate >= '2024-01-01' AND PartnerID_Valid IS NOT NULL THEN
                FIRST_VALUE(PartnerID_Valid) OVER (
                    PARTITION BY Owner
                    ORDER BY DisbursementDate, CreditID
                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        ...
        CASE 
            WHEN DisbursementDate >= '2025-01-01' AND PartnerID_Valid IS NOT NULL THEN
                FIRST_VALUE(PartnerID_Valid) OVER (
                    PARTITION BY Owner
                    ORDER BY DisbursementDate, CreditID
                    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
