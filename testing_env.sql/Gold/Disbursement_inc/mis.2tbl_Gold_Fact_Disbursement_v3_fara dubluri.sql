...
Propagated AS (
    SELECT *,
        ...
        CASE WHEN DisbursementDate >= '2023-01-01' THEN
            FIRST_VALUE(PartnerID_Valid) OVER (
                PARTITION BY Owner
                ORDER BY DisbursementDate, CreditID
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        ...
        CASE WHEN DisbursementDate >= '2024-01-01' THEN
            FIRST_VALUE(PartnerID_Valid) OVER (
                PARTITION BY Owner
                ORDER BY DisbursementDate, CreditID
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
        ...
        CASE WHEN DisbursementDate >= '2025-01-01' THEN
            FIRST_VALUE(PartnerID_Valid) OVER (
                PARTITION BY Owner
                ORDER BY DisbursementDate, CreditID
                ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
...
Dedup AS (
    SELECT *,
        ROW_NUMBER() OVER(PARTITION BY CreditID ORDER BY DisbursementDate, CreditID) AS rn
    FROM Propagated
)
SELECT *
INTO #PartnerTemp
