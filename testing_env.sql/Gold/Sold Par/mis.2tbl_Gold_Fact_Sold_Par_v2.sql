...
;WITH CreditBase AS (
    SELECT
       ...
       CASE
            WHEN rs_restruct.RestructuringDebtType = N'НекоммерческаяРеструктуризация' THEN 'Restructurizare non-comerciala'
            WHEN rs_restruct.RestructuringDebtType = N'КоммерческаяРеструктуризация' THEN 'Restructurizare comerciala'
            ELSE NULL
        END AS StickyRestructuringDebtType
    ...
    OUTER APPLY (
        SELECT TOP(1) s.[СостоянияРеструктурированныхКредитов Состояние Реструктурированного Кредита] AS RestructuredCreditState
    ...
    -- Step 2: Sticky per CreditID
    SELECT *,
        CASE 
            WHEN StickyRestructuredCreditState = 'Nevindecat' THEN 'Nevindecat'
            WHEN StickyRestructuredCreditState IS NULL THEN NULL
            ELSE 'Vindecat'
        END AS StickyCreditState,
        CASE
            WHEN StickyRestructuringDebtType = 'Restructurizare non-comerciala' THEN 'Restructurizare non-comerciala'
            ELSE StickyRestructuringDebtType
        END AS StickyDebtType
    FROM CreditBase
), ClientDayContamination AS (
    -- Step 3: Contaminate per ClientID + SoldDate
    SELECT *,
        CASE 
            WHEN MAX(CASE WHEN StickyCreditState = 'Nevindecat' THEN 1 ELSE 0 END) OVER(PARTITION BY ClientID, SoldDate) = 1
                AND StickyCreditState IS NULL THEN 'NevindecatContaminat'
            WHEN StickyCreditState = 'Nevindecat' THEN 'Nevindecat'
            ELSE StickyCreditState
        END AS ContaminatedCreditState,
        CASE 
            WHEN MAX(CASE WHEN StickyDebtType = 'Restructurizare non-comerciala' AND StickyCreditState = 'Nevindecat' THEN 1 ELSE 0 END) OVER(PARTITION BY ClientID, SoldDate) = 1
                AND StickyDebtType IS NULL THEN 'Restructurizare non-comerciala'
            WHEN StickyDebtType = 'Restructurizare non-comerciala' THEN 'Restructurizare non-comerciala'
            ELSE StickyDebtType
